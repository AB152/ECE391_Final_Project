...
void init_paging() {
    ...
    page_directory[1]/*.one_gig[1]*/.pd_mb.present = 1;    //present b/c page is being initialized
    page_directory[1]/*.one_gig[1]*/.pd_mb.read_write = 1;     //all pages are marked read/write for mp3
    page_directory[1]/*.one_gig[1]*/.pd_mb.user_supervisor = 0;    //0 for kernel pages
    page_directory[1]/*.one_gig[1]*/.pd_mb.page_write_through = 0;    //we always want writeback, so 0
    page_directory[1]/*.one_gig[1]*/.pd_mb.page_cache_disabled = 1;    //1 for program code and data pages (kernel pages)
    page_directory[1]/*.one_gig[1]*/.pd_mb.accessed = 0;   //not used at all in mp3
    page_directory[1]/*.one_gig[1]*/.pd_mb.dirty = 0;      //not used at all in mp3
    page_directory[1]/*.one_gig[1]*/.pd_mb.page_size = 1;  //1 if 4M page directory entry
    page_directory[1]/*.one_gig[1]*/.pd_mb.global_bit = 1; // the kernel is mapped into every process’s address space
    page_directory[1]/*.one_gig[1]*/.pd_mb.available = 0;  //not used at all in mp3
    page_directory[1]/*.one_gig[1]*/.pd_mb.page_attr_index = 0;  //not used at all in mp3
    page_directory[1]/*.one_gig[1]*/.pd_mb.reserved = 0;       //reserved bits are always set to 0
    page_directory[1]/*.one_gig[1]*/.pd_mb.base_addr = 1;      // Kernel is at first 4MB
    ...
    //for(j=0;j<4;j++){ 
        for(i=2;i<ONE_KB;i++){
            ...
            //if(j!=0 && i>2){
                ...
                page_directory[i]/*.one_gig[i]*/.pd_mb.user_supervisor = 0;
                page_directory[i]/*.one_gig[i]*/.pd_mb.page_write_through = 0;
                page_directory[i]/*.one_gig[i]*/.pd_mb.page_cache_disabled = 0;
