...
int32_t halt(uint8_t status){
    ...
    cli();
    pcb_t *pcb_ptr = tss.esp0  & 0xFFFFE000;
    //initalize pcb
    int i;
    for(i = 0; i < 8; i++) {
...
int32_t execute(const uint8_t* command){
    ...
    int val = read_data(file_dentry.inode, 0, PROG_IMG_ADDR, exec_length);
    if(val == -1){
    ...
    // Mark PID as in use in array (IMPORTANT: FIGURE OUT HOW TO IMPLEMENT PARENT_PID)
...
int32_t read(int32_t fd, void* buf, int32_t nbytes){
    /* IMPORTANT */
    pcb_t *pcb = tss.esp0  & 0xFFFFE000; //ANDing the process's ESP register w/ appropriate bit mask to reach top of stack
...
int32_t write(int32_t fd, const void* buf, int32_t nbytes){
    pcb_t *pcb=tss.esp0  & 0xFFFFE000;;    //temp placeholder until we figure out how to initialize the pcb
    if(fd<0 || fd>7) //check for valid fd index, max 8 files
        return -1;
